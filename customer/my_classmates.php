													...
													<tbody class="tr-sortable">
														...
														?>
															...
															<tr class="bg-white">
																...
																<td colspan="2" class=" p-1" style="background:#039715;width:50px;"><b><?php echo  $c* $row['weight'] /5 ?></b></td>
															</tr>
...
<body>
 ...
 <br/>
 <textarea name="suggestion" rows="8" cols="100" style="width:377px;height:46px;"></textarea>
 </div>
  <button name="login" class="btn btn-info" type="submit"><i class="icon-signin icon-"></i> Send Result</button>
						...
						if (isset($_POST['login'])) {
							...
							$percent = "100*";
							$query = mysqli_query($conn, "select * from employee_result where emplo_id = '$get_id' and program_id ='$program_id' ") or die(mysqli_error());
							$count = mysqli_num_rows($query);
							if ($count > 0) { ?>
							 <script>
							   alert('በድጋሜ መመዘን አልተፈቀደም!!');
							 </script>
						   <?php
							} else {
							  mysqli_query($conn, "insert into employee_result (employee_id,emplo_id,result_point,Date_Time,Time,program_id,stated) values('$session_id','$get_id','$overall_result',NOW(),NOW(),'$program_id','$wate')") or die(mysqli_error());
